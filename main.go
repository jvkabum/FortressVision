...
func main() {
	...
	dfhackHost := flag.String("host", "", "Host do DFHack (padrão: localhost)")
	dfhackPort := flag.Int("port", 0, "Porta do DFHack (padrão: 5000)")
	...
	if *dfhackHost != "" {
		cfg.DFHackHost = *dfhackHost
	}
	if *dfhackPort != 0 {
		cfg.DFHackPort = *dfhackPort
	}
	if *fullscreen {
		cfg.Fullscreen = true
	}
