...
// getMaterialColor mapeia um MaterialID para uma cor de vértice estável.
func getMaterialColor(matID int32) matrix.Color {
	r := matrix.Float((matID*123)%255) / 255.0
	g := matrix.Float((matID*456)%255) / 255.0
	b := matrix.Float((matID*789)%255) / 255.0
	return matrix.NewColor(r, g, b, 1.0)
}
...
func meshGreedy2D(chunk *mapdata.Chunk, md *MeshData) {
	...
	for y := 0; y < 16; y++ {
		for x := 0; x < 16; x++ {
			...
			matVal := tile.Material.MatIndex + 1
	...
	for faceDir := 0; faceDir < 5; faceDir++ {
		for y := 0; y < 16; y++ {
			for x := 0; x < 16; x++ {
				...
				color := getMaterialColor(matVal - 1)
...
func meshSpecials(chunk *mapdata.Chunk, md *MeshData) {
	for y := 0; y < 16; y++ {
		for x := 0; x < 16; x++ {
			...
			shape := tile.Shape()
			color := getMaterialColor(tile.Material.MatIndex)
